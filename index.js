...
const opt = require('./secret.json')
const {SpotifyToken} = require('./spoty_token')

let socketServer = http.createServer()
const io = require('socket.io')(socketServer)

const spoty = new SpotifyToken(opt.spotify_client_id,opt.spotify_client_secret)
...
let urlCallbackDict = {
    ...
    "/overlay": (req,res,body,query)=>{
        if (query['sdf']!=opt.secret_cookie){
            res.writeHead(403)
            res.end()
        }
        else{
    ...
    "/overlay/twitch_request.js": (req,res,body,query)=>{
        if (query['sdfgfd']!=opt.secret_cookie2){
            res.writeHead(403)
            res.end()
        }
        else{
    ...
    "/spoty/header":(req,res,body,query)=>{
        if (query['secret']!=opt.secret_cookie){
            res.writeHead(403)
            res.end()
        }
        else{
            res.writeHead(200)
            res.end(JSON.stringify(spoty.get_bearer_header()))
        }
    },
}
